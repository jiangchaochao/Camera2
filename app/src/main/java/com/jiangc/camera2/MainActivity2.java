    ...
    SurfaceHolder.Callback callback = new SurfaceHolder.Callback() {
        ...
        public void surfaceCreated(@NonNull SurfaceHolder holder) {
            ...
            try {
                 ...
                 * */
                try {
                    manager.openCamera(CameraList[1], mCameraDeviceStateCallback, mhandler);
                } catch (Exception e) {
                    e.printStackTrace();
                }
...
//            Surface surface = new Surface(texture);
            ...
            mCaptureBuilder.addTarget(surfaceView.getHolder().getSurface());
            try {
                /*创建这个会话用来接收请求的数据*/
                camera.createCaptureSession(Arrays.asList(surfaceView.getHolder().getSurface()), CameraStateCallback, mhandler);
            } catch (CameraAccessException e) {
                e.printStackTrace();
            }
        }
